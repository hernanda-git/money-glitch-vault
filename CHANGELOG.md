...
## 2026-07-14 -- enrich(01): x search operators playbook -- 867-line technical reference for the X/Twitter web advanced-search operator grammar (content, user, geo, time, type, engagement, media classes), with the critical finding that these operators do NOT work on the paid v2 API (pay-per-use, owned-reads ~$0.001, 7-day window only), which justifies a twscrape-based scraper. Includes working Python (twscrape API, query-builder module, daily-cron orchestration script), 30 decoded combinatorial examples with encoded URLs, normalized intake JSON schema, troubleshooting table, and 9 drop-in vault pain-point queries. Sourced from igorbrigadir/twitter-advanced-search, vladkens/twscrape, Luca Hammer guide, and X API docs. New gap discovered: verification_type field needed to distinguish legacy vs blue_verified in normalized records.
